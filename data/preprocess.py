...
def getData(path='./dataset/', dataset='SWaT', period=1440, train_rate=0.8):
    ...
    batch_mu_train = np.load(path + dataset + '/' + dataset + '_train_batch_mu.npy')  # Train set injected anomalies
    batch_mu_test = np.load(path + dataset + '/' + dataset + '_test_batch_mu.npy')    # Test set injected anomalies
    ...
    train_mu = batch_mu_train[:int(train_rate * len(batch_mu_train)), :]
    ...
    valid_mu = batch_mu_train[int(train_rate * len(batch_mu_train)):, :]
    ...
    data = {
        ...
        'test_mu': batch_mu_test  # 加入 test_mu
    }
